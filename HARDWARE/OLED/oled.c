...
extern const unsigned char BMP1[1024];
/****************************************************************
...
void OLED_ShowChinese(u8 x,u8 y,u8 num,u8 size1,u8 mode)
{
    ...
    for(i=0; i<size3; i++)
    {
        ...
        {
            temp=Hzk2[num][i];   //调用12*12字体
        }
        else if(size1==8)
        {
            temp=Hzk3[num][i];   //调用8*8字体
        }
//        else if(size1==64)
//        {
...
void OLED_ShowPicture(u8 x, u8 y, u8 sizex, u8 sizey, u8 BMP[], u8 mode)
{
    u16 j = 0;          // BMP数组的当前索引
    u8 i, n, temp, m;   // 临时变量
    u8 x0 = x, y0 = y;  // 起点坐标的副本

    // 计算图片在纵向需要占据的页数，每页8像素
    sizey = sizey / 8 + ((sizey % 8) ? 1 : 0);
